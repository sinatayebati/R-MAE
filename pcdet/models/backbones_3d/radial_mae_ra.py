...
class Radial_MAE_ra(nn.Module):
    """
    pre-trained model with radial masking
    """
    ...
    def radial_range_aware_masking(self, voxel_coords):
        ...
        voxel_coords_distance = (voxel_coords[:, 2]**2 + voxel_coords[:, 3]**2)**0.5  # Calculate the Euclidean distance of each voxel from the origin (0,0) in the XY plane

        # Group indices based on angles
        angular_range = self.angular_range # degrees, can be adjusted in config, refer to YAML files for adjustment
        radial_groups = {}
        ...
        for angle in selected_group_angles:
            ...
            id_list_select_30 = group_indices[select_30]
            id_list_select_30to50 = group_indices[select_30to50]
            id_list_select_50 = group_indices[select_50]

            random.shuffle(id_list_select_30)
            random.shuffle(id_list_select_30to50)
            random.shuffle(id_list_select_50)

            # Concatenate selected indices from each distance range
            selected_indices.extend(id_list_select_30[:int(select_ratio * len(id_list_select_30))])
            selected_indices.extend(id_list_select_30to50[:int((select_ratio + 0.2) * len(id_list_select_30to50))])
            selected_indices.extend(id_list_select_50[:int((select_ratio + 0.4) * len(id_list_select_50))])

        ## Convert list to 1-dimensional tensor and return
    ...
    def forward(self, batch_dict):
        ...
        selected_indices_tensor = self.radial_range_aware_masking(voxel_coords)

        # nums is set to voxel_features.shape[0], which is the number of voxels
