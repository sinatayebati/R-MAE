...
class Voxel_MAE(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        id_list_select_30 = torch.nonzero(select_30==True).reshape(torch.nonzero(select_30==True).shape[0])
        id_list_select_30to50 = torch.nonzero(select_30to50==True).reshape(torch.nonzero(select_30to50==True).shape[0])
        id_list_select_50 = torch.nonzero(select_50==True).reshape(torch.nonzero(select_50==True).shape[0])
